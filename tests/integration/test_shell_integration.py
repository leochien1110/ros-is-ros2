...
class TestShellIntegration:
    ...
    def bash_script_path(self):
        ...
        return Path(__file__).parent.parent.parent / "src" / "ros_is_ros2" / "shims" / "bash.sh"

    @pytest.fixture
    def zsh_script_path(self):
        """Get path to zsh shim script."""
        return Path(__file__).parent.parent.parent / "src" / "ros_is_ros2" / "shims" / "zsh.sh"
    ...
    def test_bash_aliases_defined(self, bash_script_path):
        ...
        bash_commands = [
            ...
            "alias | grep -E '^(rostopic|rosnode|rosservice)=' || echo 'No aliases found'"
        ]
        
        result = subprocess.run(
            ["bash", "-c", "; ".join(bash_commands)],
            capture_output=True,
            text=True
        )
        
        # Check if aliases were defined (may fail if bash-completion missing)
        if "No aliases found" in result.stdout:
    ...
    def test_complete_alias_script_exists(self):
        ...
        complete_alias_path = (
            ...
            / "src" / "ros_is_ros2" / "third_party" / "complete_alias.bash"
        )
    ...
    def test_complete_alias_syntax(self):
        ...
        result = subprocess.run(
            ["bash", "-n", str(complete_alias_path)],
            capture_output=True,
            text=True
        )
        assert result.returncode == 0, f"complete-alias.bash syntax error: {result.stderr}"
